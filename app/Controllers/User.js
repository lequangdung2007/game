									...
									try {
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date()}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'Tham khảo thêm nhiều game bài đổi thưởng tại MuaGameBai.Com', 'time':new Date()});
												GameState(client);
												client.red(data);
											}
										});
